...
- `词表` 页面显示当前词库，支持通过管理弹窗选择/删除词表、导入 CSV、导入 PDF、搜索、查看状态和删除选中词条。
- AI 例句补全使用 `model/` 目录中的本地 `.gguf` 模型；补充释义使用纯 Python 的 Argos Translate，不启动 GGUF 模型。

## 数据

- 启动时读取 `input/` 中当前选中的 CSV；`input/` 可以同时保存多个 CSV。
- 导入 CSV 会先校验列名，通过后复制到 `input/` 并切换到该词表。导入 PDF 会自动解析并在 `input/` 中生成一个 CSV。
...
- 导入 PDF 会提取文本、使用确定性的多语言频率引擎清洗文本、识别语言、统计 lemma 频率和词形，并生成可导入的 PDF 词表。扫描图片 PDF 需要先 OCR 成可复制文本。
- 导入 PDF 时会询问是否使用本地 AI 模型检查 PDF 生成的词表；选择“是”会使用或下载 `model/` 中的默认 `.gguf` 模型来删除不是单词或词组的条目，选择“否”则直接使用未经 AI 检查的词表。PDF 基础转换不使用外部 API 或 transformer pipeline。
- SQLite 保存词条、例句、会/不会次数、复习时间和学习状态。
- 正常启动会按 CSV 更新单词释义、频率和词形，不会清空已有例句或学习进度。
- `重置学习进度` 会保留词表内容、例句和例句翻译，只清空学习进度。
...
- 点击 `不认识`：不认识次数加 1。
- 点击 `认识`：认识次数加 1；累计到 5 次后进入复习池。
- 点击 `很熟`：直接把认识次数提升到至少 5，并进入复习池。
...
- 只有点击过 `不认识`、`认识` 或 `很熟` 的词会进入本次会话历史。
- 历史词按操作顺序保存，可用 `上一个` / `下一个` 顺序浏览。
- 浏览历史词时不显示 `不认识`、`认识`、`很熟`，避免重复计数。
- 在历史末尾点击 `下一个`，会继续抽取本次会话中还没有显示过的新词。

## 本地模型

- AI 例句补全使用 `model/` 目录中的 `.gguf` 模型；补充释义使用 Argos Translate 本地翻译包。
- `model/` 目录中只能存在一个 `.gguf` 文件；如果存在多个，AI 功能会停止并提示清理。
- 如果 `model/` 中没有 `.gguf` 文件，首次使用 AI 功能时会自动从 Hugging Face 下载默认模型：
  `Qwen/Qwen2.5-3B-Instruct-GGUF/qwen2.5-3b-instruct-q4_k_m.gguf`。
- 如果用户自行放入 `.gguf` 模型，软件会优先使用用户模型，并在 AI 功能启动前提示兼容性不保证。
